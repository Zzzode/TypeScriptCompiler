...
{
public:
    LLVMDebugInfoHelper(MLIRContext *context, LLVMTypeConverterHelper llvmtch) :
        context(context), llvmtch(llvmtch)
    {
    }
    ...
    LLVM::DITypeAttr getDILLVMType(mlir::Type llvmType, mlir::Type type, LLVM::DIFileAttr file, uint32_t line, LLVM::DIScopeAttr scope)
    {
        ...
        mlir::TypeSwitch<mlir::Type>(llvmType)
                .Case<mlir::IntegerType>([&](auto intType) {
                    ...
                    diTypeAttr = LLVM::DIBasicTypeAttr::get(context, dwarf::DW_TAG_base_type, StringAttr::get(context, typeName), size,
                                                            typeCode);
                })
                .Case<mlir::FloatType>([&](auto floatType) {
                    ...
                    diTypeAttr = LLVM::DIBasicTypeAttr::get(context, dwarf::DW_TAG_base_type, StringAttr::get(context, typeName),
                                                            floatType.getIntOrFloatBitWidth(), dwarf::DW_ATE_float);
                })
...
private:
    LLVM::DITypeAttr getDIPointerType(LLVM::LLVMPointerType llvmPointerType, mlir::Type elementType, LLVM::DIFileAttr file, uint32_t line,
    ...
    LLVM::DITypeAttr getDIStructType(LLVM::LLVMStructType structType, mlir::Type type, LLVM::DIFileAttr file, uint32_t line,
                                     LLVM::DIScopeAttr scope)
    {
        MLIRTypeHelper mth(context);
        ...
        for (auto llvmElementType: structType.getBody())
        {
            ...
            auto wrapperDiType = LLVM::DIDerivedTypeAttr::get(context, dwarf::DW_TAG_member, name, elementDiType,
                                                              sizesTrack.elementSizeInBits, sizesTrack.elementAlignInBits,
                                                              sizesTrack.offsetInBits);
            elements.push_back(wrapperDiType);
        }

        return LLVM::DICompositeTypeAttr::get(context, dwarf::DW_TAG_structure_type,
                                              StringAttr::get(context, MLIRHelper::getAnonymousName(structType, "struct")),
                                              file, line, scope, LLVM::DITypeAttr(), LLVM::DIFlags::TypePassByValue, sizesTrack.sizeInBits,
                                              sizesTrack.alignInBits, elements);
    }

    LLVM::DITypeAttr getDIStructType(StringRef name, ArrayRef<std::pair<StringRef, mlir::Type>> fields, LLVM::DIFileAttr file,
                                     uint32_t line, LLVM::DIScopeAttr scope)
    {
        MLIRTypeHelper mth(context);
    ...
    LLVM::DITypeAttr getDIStructType(StringRef name, ArrayRef<std::pair<StringRef, LLVM::DITypeAttr>> fields, LLVM::DIFileAttr file,
                                     uint32_t line, LLVM::DIScopeAttr scope)
    {
        MLIRTypeHelper mth(context);
        ...
        for (auto field: fields)
        {
            ...
            LLVM::DITypeAttr elementDiType = std::get<1>(field);

            sizesTrack.nextElementType(elementDiType);

            LLVM::DIDerivedTypeAttr wrapperDiType = LLVM::DIDerivedTypeAttr::get(context, dwarf::DW_TAG_member, name, elementDiType,
                                                                                 sizesTrack.elementSizeInBits,
                                                                                 sizesTrack.elementAlignInBits,
                                                                                 sizesTrack.offsetInBits);
            elements.push_back(wrapperDiType);
        }
    ...
    LLVM::DITypeAttr getDIUnionType(mlir_ts::UnionType unionType, LLVM::DIFileAttr file, uint32_t line, LLVM::DIScopeAttr scope)
    {
        ...
        for (auto elementType: unionType.getTypes())
        {
            ...
            auto llvmElementType = llvmtch.typeConverter.convertType(elementType);
            sizesTrack.nextElementType(llvmElementType);
            if (sizesTrack.elementSizeInBits > sizeInBits)
                sizeInBits = sizesTrack.elementSizeInBits;
            ...
            auto wrapperDiType = LLVM::DIDerivedTypeAttr::get(context, dwarf::DW_TAG_member, name,
                                                              elementDiType, sizesTrack.elementSizeInBits, sizesTrack.elementAlignInBits,
                                                              0);
            elements.push_back(wrapperDiType);
        }

        return LLVM::DICompositeTypeAttr::get(context, dwarf::DW_TAG_union_type,
                                              StringAttr::get(context, MLIRHelper::getAnonymousName(unionType, "union")),
                                              file, line, scope, LLVM::DITypeAttr(), LLVM::DIFlags::TypePassByValue, sizeInBits,
                                              sizesTrack.alignInBits, elements);
    }
